...
test_that("an interrupted run resumes without rescoring committed work", {
  ...
  crash_after(mock, 2L)
  expect_error(score_texts(d$texts, d$ids, task = prepared, engine = engine, chunk_size = 5L, checkpoint = run, progress = FALSE), "simulated crash")
  ...
  mock$calls <- list()
  expect_error(score_texts(d$texts, d$ids, task = prepared, engine = engine, checkpoint = run, progress = FALSE), "already exists")
  result <- score_texts(d$texts, d$ids, task = prepared, engine = engine, chunk_size = 5L, checkpoint = run, resume = TRUE, progress = FALSE)
...
test_that("resume retries only retryable failures, and a complete run sends nothing", {
  ...
  d <- dataset()
  run <- file.path(withr::local_tempdir(), "run")
  score_texts(d$texts, d$ids, task = prepared, engine = engine, checkpoint = run, progress = FALSE)
  mock$calls <- list()
  score_texts(d$texts, d$ids, task = prepared, engine = engine, checkpoint = run, resume = TRUE, progress = FALSE)
  expect_identical(mock$score_calls(), list("r22"))

  keep <- d$ids %in% c("r01", "r02", "r03", "r05")
  run2 <- file.path(withr::local_tempdir(), "run2")
  score_texts(d$texts[keep], d$ids[keep], task = prepared, engine = engine, checkpoint = run2, progress = FALSE)
  mock$calls <- list()
  again <- score_texts(d$texts[keep], d$ids[keep], task = prepared, engine = engine, checkpoint = run2, resume = TRUE, progress = FALSE)
...
test_that("resume rejects changed inputs, order, and task", {
  ...
  expect_error(score_texts(changed, d$ids, task = prepared, engine = engine, checkpoint = run, resume = TRUE, progress = FALSE), "texts")
  expect_error(score_texts(rev(d$texts), rev(d$ids), task = prepared, engine = engine, checkpoint = run, resume = TRUE, progress = FALSE), "order")
  numeric <- prepare_task(quickstart_numeric(), engine)
  expect_error(score_texts(d$texts, d$ids, task = numeric, engine = engine, checkpoint = run, resume = TRUE, progress = FALSE), "task")
})
...
test_that("locks block concurrent writers until forced", {
  ...
  d <- dataset()
  run <- file.path(withr::local_tempdir(), "run")
  score_texts(d$texts[1:3], d$ids[1:3], task = prepared, engine = engine, checkpoint = run, progress = FALSE)
  ...
  expect_error(score_texts(d$texts[1:3], d$ids[1:3], task = prepared, engine = engine, checkpoint = run, resume = TRUE, progress = FALSE), "locked")
  score_texts(d$texts[1:3], d$ids[1:3], task = prepared, engine = engine, checkpoint = run, resume = TRUE, force_unlock = TRUE, progress = FALSE)
  expect_false(dir.exists(file.path(run, ".lock")))
})
...
test_that("a crash before commit rescores that chunk; stray temp files are ignored", {
  ...
  expect_error(score_texts(d$texts, d$ids, task = prepared, engine = engine, chunk_size = 5L, checkpoint = run, progress = FALSE), "disk full")
...
test_that("checkpoints contain no texts or credentials; other directories are not reused", {
  ...
  expect_error(score_texts("a", "r01", task = prepared, engine = engine, checkpoint = other, progress = FALSE), "not a llikert checkpoint")
})
