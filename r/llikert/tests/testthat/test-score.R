...
test_that("zero rows, one row, and all-failed rows", {
  ...
  empty <- score_items(character(), task = prepared, engine = engine, progress = FALSE)
  expect_identical(nrow(empty), 0L)
  expect_identical(names(empty), c("id", "description", "question", "request"))
  expect_length(mock$score_calls(), 0)
  one <- score_items("Where is the nearest train station?", "r01", task = prepared, engine = engine, progress = FALSE)
  expect_identical(llikert_result_diagnostics(one)$status, "ok")
  failed <- score_items(c(NA, ""), c("r04", "r07"), task = prepared, engine = engine, progress = FALSE)
...
test_that("ids are validated and converted exactly", {
  ...
  engine <- scorer_connect("http://mock")
  prepared <- prepare_task(quickstart_nominal(), engine)
  s <- function(...) score_items(task = prepared, engine = engine, progress = FALSE, ...)
  ...
  expect_error(score_items("a", task = quickstart_nominal(), engine = engine), "prepare")
})
...
test_that("a stale prepared task is rejected before any request", {
  ...
  expect_error(score_items("x", task = prepared, engine = engine, progress = FALSE), class = "llikert_error_engine_fingerprint_mismatch")
  expect_length(mock$score_calls(), 0)
})
...
test_that("latin1 input is transcoded and invalid bytes are reported per item", {
  ...
  result <- score_items(bad, "bad", task = prepared, engine = engine, progress = FALSE)
...
test_that("non-JSON errors are summarized without markup", {
  ...
  cnd <- expect_error(score_items("a", "r01", task = prepare_task(quickstart_nominal(), engine), engine = engine, progress = FALSE),
...
test_that("permanent errors are not retried", {
  for (case in list(list(400L, "invalid_json"), list(409L, "prepared_task_mismatch"), list(422L, "invalid_request"))) {
    ...
    expect_error(score_items("a", "r01", task = prepared, engine = engine, progress = FALSE), class = paste0("llikert_error_", case[[2]]))
...
test_that("connection failures are retried a bounded number of times", {
  ...
  expect_error(score_items("a", "r01", task = prepared, engine = engine, progress = FALSE), class = "llikert_error_transport")
