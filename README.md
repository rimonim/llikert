...
**Status:** pre-release (v0.1 development). The scoring service (M1), the R and Python clients (M2), and the benchmarked, fidelity-gated execution path (M3), and the verified service container (M4) work locally. Hosted deployment is documented but not yet executed; the release review (M5) is next.

- `llikert_development_plan.md` — product specification
- `docs/implementation-plan.md` — detailed plan and decisions
- `docs/decisions/` — decision records (M0 feasibility, M1 service, M2 clients and CUDA toolchain, M3 performance and fidelity, M4 distribution)
- `docs/protocol.md` — HTTP protocol v1
- `docs/local-service.md` — running the service on a host
- `deploy/README.md` — the service container; `deploy/hf-endpoint.md` — Hugging Face recipe (experimental, not executed)
- `docs/troubleshooting.md` — common errors and what to do
